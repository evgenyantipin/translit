...
module Translit
  def self.convert!(text, enforce_language = nil)
    ...
    map = self.send("translit_#{language}").sort_by {|k,v| v.length <=>  k.length}
...
private
  def self.create_russian_map
    self.translit_english.inject({}) do |acc, tuple|
